...
Ideally, by compartmentalizing each attribute with an id and a value, it allows for infinite categories to be made and looked through efficiently. Storing the IDs seperately allows for a full reference of all elements in storage and if an ID is not in a category list it can be considered to having no attribute or a zero attribute associated with it.
